...
const MenuNavbarMobil = () => {
  ...
  return (
    <nav className="shadow-md  top-0 left-0 z-50">
      ...
      {/* link items */}
      <div
        className={`md:flex md:items-center md:pb-0 absolute md:static bg-white left-0 w-[90%] md:pl-0 transition-all duration-500 ease-in ${
      ...
      >
        <ul className="flex flex-col space-y-2 py-4 border-b border-gray-200 bg-white text-left">
          {MenuDataLinks.map((data, index) => (
            <li className="w-full font-bold" key={data.id}>
              <Link
                to="/"
              ...
              >
                ...
                {index !== MenuDataLinks.length - 1 && (
                  <HiChevronRight className="ml-auto text-black" />
                )}
              </Link>
            </li>
          ))}
        ...
        <ul className="flex flex-col w-full space-y-2 py-4 bg-gray-100 text-left">
          {MenuInfoLinks.map((info) => (
            <li className="w-full font-semibold" key={info.id}>
              <Link
                to="/"
                className="flex items-center px-4 py-2 text-black hover:bg-gray-200 text-sm w-full"
              >
                {info.title}
              </Link>
            </li>
